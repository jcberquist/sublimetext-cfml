...
def get_method_documentation(view, extended_metadata, file_path, function_name, class_name, method_name, method_preview):
    model_doc = build_method_documentation(extended_metadata, function_name, class_name, method_name, method_preview)
    callback = partial(on_navigate, view, file_path, extended_metadata["function_file_map"])
    return model_doc, callback


def get_method_preview(view, extended_metadata, file_path, function_name, method_preview):
    model_doc = build_method_preview_doc(extended_metadata, function_name, method_preview)
    callback = partial(on_navigate, view, file_path, extended_metadata["function_file_map"])
    return model_doc, callback


def get_function_call_params_doc(extended_metadata, function_call_params, class_name, method_name):
    model_doc = build_function_call_params_doc(extended_metadata, function_call_params, class_name, method_name)
...
def build_documentation(extended_metadata, file_path, class_name):
    ...
    if "hint" in extended_metadata and extended_metadata["hint"]:
        model_doc["html"]["body"] += documentation_helpers.card(body=extended_metadata["hint"])

    if "entityname" in extended_metadata and extended_metadata["entityname"]:
        header = documentation_helpers.header("entityname", extended_metadata["entityname"], "string.quoted")
        model_doc["html"]["body"] += documentation_helpers.card(header)

    if "extends" in extended_metadata and extended_metadata["extends"]:
        header = documentation_helpers.header("extends", extended_metadata["extends"], "entity.other.inherited-class")
        model_doc["html"]["body"] += documentation_helpers.card(header)
    ...
    if len(extended_metadata["functions"]) > 0:
        ...
        if "constructor" in functions:
            ...
            model_doc["html"]["body"] += documentation_helpers.card(*functions["constructor"])
...
def build_method_documentation(extended_metadata, function_name, class_name, method_name, method_preview=None):
    ...
    if class_name:
        model_doc["html"]["header"] += documentation_helpers.span_wrap(class_name, "entity.name.class")
        if method_name:
            model_doc["html"]["header"] += "."
    if method_name:
        model_doc["html"]["header"] += documentation_helpers.span_wrap(method_name, "entity.name.function") + "()"
    if funct["meta"]["access"] and len(funct["meta"]["access"]) > 0:
        model_doc["html"]["header"] = documentation_helpers.span_wrap(funct["meta"]["access"], "storage.modifier") + " " + model_doc["html"]["header"]
    if funct["meta"]["returntype"] and len(funct["meta"]["returntype"]) > 0:
        model_doc["html"]["header"] += ": " + documentation_helpers.span_wrap(funct["meta"]["returntype"], "storage.type")

    model_doc["html"]["body"] = """
    <div class="path">
        <strong>path</strong>: <a href="{}">{}</a>
    </div>
    """.strip().format(funct["name"], function_file_path)

    if "hint" in funct["meta"] and funct["meta"]["hint"]:
        model_doc["html"]["body"] += documentation_helpers.card(body=funct["meta"]["hint"])
    ...
    if len(funct["meta"]["parameters"]) > 0:
        ...
        for arg in funct["meta"]["parameters"]:
            ...
            body = ""
            if arg["default"]:
                body += "<p><em>Default:</em> <span class=\"code\">" + str(arg["default"]) + "</span></p>"
...
def build_method_preview_doc(extended_metadata, function_name, method_preview):
    ...
    preview["html"]["link"] = "<strong>path</strong>: <a class=\"plain-link\" href=\"" + funct["name"] + "\">" + function_file_path + "</a>"
    css = method_preview["css"].replace("<style>", "").replace("</style>", "")
...
def build_function_call_params_doc(extended_metadata, function_call_params, class_name, method_name):
    model_doc = {"side_color": SIDE_COLOR, "html": {}}
    ...
    if len(funct["meta"]["parameters"]) > 0:
        for index, arg in enumerate(funct["meta"]["parameters"]):
            ...
            if is_active:
                model_doc["html"]["body"] += "type: " + documentation_helpers.span_wrap((arg["type"] if arg["type"] else "any"), "storage.type") + "<br>"
...
def build_param_header(param):
    args = {"name": param["name"], "type": "", "required": ""}
    if "type" in param and param["type"] and len(param["type"]):
        args["type"] = ": " + documentation_helpers.span_wrap(param["type"], "storage.type")
    if "required" in param and param["required"]:
        args["required"] = " <span class=\"required\">&nbsp;Required&nbsp;</span>"
    return PARAM_TEMPLATE.format(**args)
...
def parse_function(function, funct_file_path, file_path):
    ...
    header += documentation_helpers.span_wrap(function["name"], "entity.name.function") + "(" + ("..." if function["meta"]["parameters"] else "") + ")"
    if function["meta"]["returntype"]:
        header += ": " + documentation_helpers.span_wrap(function["meta"]["returntype"], "storage.type")
    header += "</a>"
...
def parse_property(prop, prop_file_path, file_path):
    header = "<strong>" + prop["name"] + ": " + documentation_helpers.span_wrap(prop["meta"]["type"], "storage.type") + "</strong>"
    if prop_file_path != file_path:
        header += " <small><em>(from " + prop_file_path.split("/")[-1] + ")</em></small>"

    body = ""
    accessors = [key for key in ["setter", "getter"] if prop["meta"][key]]
    if accessors:
        body += "<small><strong>accessors</strong>: <em>" + ", ".join(accessors) + "</em></small>"
