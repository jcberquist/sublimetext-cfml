...
def indent_region(cfml_format):
    ...
    for l in lines:
        ...
        if first_line_char == ',' and cfml_format.view.match_selector(first_char_pt, 'punctuation.separator'):
            if not leading_comma_flag:
                # move previous line over to align
                offset = 2 if stripped_line_str[1] == ' ' else 1
                replacements[-1] = cfml_format.indent_to_column(indent_columns + offset) + replacements[-1].strip()
                leading_comma_flag = True
        else:
            leading_comma_flag = False

        # lines that start with a method call get an extra indent
        if first_line_char == '.' and cfml_format.view.match_selector(first_char_pt, accessor_selector):
...
def align_assignment_operators(cfml_format):
    ...
    def max_start(seq):
        offsets = [op_region.begin() - line_region.begin() for op_region, op_scope, line_region in seq]
        return max(offsets)

    def align_line(op_region, op_scope, line_region, offset):
        line_start = cfml_format.view.substr(sublime.Region(line_region.begin(), op_region.begin())).rstrip()
    ...
    def align_sequence(seq):
        ...
        for op_region, op_scope, line_region in seq:
            seq_replacements.append(align_line(op_region, op_scope, line_region, offset))
