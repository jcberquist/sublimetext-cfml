...
def format_blocks(cfml_format):
    ...
    for block_region in blocks:
        ...
        if block.start_to_char.end() == block.char_to_end.end():
            empty_spacing = cfml_format.get_setting('blocks.empty_spacing', block.block_type)
            ...
            # block start
            start_spacing = cfml_format.get_setting('blocks.start_spacing', block.block_type)
            ...
            # block end
            end_spacing = cfml_format.get_setting('blocks.end_spacing', block.block_type)
        ...
        after_block_spacing = cfml_format.get_setting('blocks.after_block_spacing', block.block_type)
...
def parse_block(cfml_format, block_region, groups):
    ...
    if prev_char_pt + 1 in groups:
        prev_char_pt = utils.get_previous_character(cfml_format.view, groups[prev_char_pt + 1])
    ...
    first_block_char = block_region.begin() + 1 + (len(block_str) - len(block_str.lstrip()))
    start_to_char = sublime.Region(block_region.begin() + 1, first_block_char)

    last_block_char = block_region.end() - 1 - (len(block_str) - len(block_str.rstrip()))
