...
def format_blocks(cfml_format):
    if not cfml_format.get_setting("blocks"):
        return []
    ...
    for block_region in blocks:
        ...
        block = parse_block(cfml_format, block_region)
        block_settings = cfml_format.get_setting("blocks", block.block_type)

        indent_column = cfml_format.line_indent_column(block.indent_pt)
        indent_str = cfml_format.indent_to_column(indent_column)
        sub_indent_str = cfml_format.indent_to_column(indent_column + cfml_format.tab_size)
        ...
        if block.start_to_char.end() == block.char_to_end.end():
            empty_spacing = block_settings.get("empty_spacing", None)
            ...
            # block start
            start_spacing = block_settings.get("start_spacing", None)
            ...
            # block end
            end_spacing = block_settings.get("end_spacing", None)
        ...
        after_block_spacing = block_settings.get("after_block_spacing", None)
        if after_block_spacing is None or after_block_spacing not in ["newline", "emptyline"]:
            continue
        ...
        if after_block_keyword and after_block_keyword in ["else", "elseif", "while", "catch"]:
            continue

        # after block spacing ignored inside delimited scopes
        if cfml_format.view.match_selector(block_region.begin(), ",".join(DELIMITED_SCOPE_LIST)):
            continue
...
def parse_block(cfml_format, block_region):
    ...
    if cfml_format.view.match_selector(prev_char_pt, GROUP_SELECTOR):
        group_start_pt = get_group_start(cfml_format, prev_char_pt)
        prev_char_pt = utils.get_previous_character(cfml_format.view, group_start_pt)

    block_type = get_block_type(cfml_format, prev_char_pt)

    indent_pt = prev_char_pt if block_type != "anonymous" else block_region.begin()

    first_block_char = utils.get_next_character(cfml_format.view, block_region.begin() + 1)
    start_to_char = sublime.Region(block_region.begin() + 1, first_block_char)

    last_block_char = utils.get_previous_character(cfml_format.view, block_region.end() - 1)
    char_to_end = sublime.Region(last_block_char + 1, block_region.end() - 1)
...
def get_block_type(cfml_format, pt):
    if cfml_format.view.match_selector(pt, "entity.name.function,storage.type.function"):
        return "function"
...
def get_keyword(cfml_format, pt):
    if cfml_format.view.match_selector(pt, "keyword.control"):
        ...
        if keyword == "else" and cfml_format.view.match_selector(pt + 4, "keyword.control"):
            keyword = "elseif"
        return keyword
    return None
