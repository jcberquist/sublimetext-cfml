...
DELIMITED_SCOPE_STARTS = [e['scope'] + ' ' + e['start'] for k, e in DELIMITED_SCOPES.items()]
...
def format_delimited_scopes(cfml_format):
    ...
    start_scope_list = cfml_format.view.scope_name(cfml_format.region_to_format.begin() - 1).rstrip().split(' ')
    ...
    for pt in region_starts:
        ...
        formatted_str = render_delimited_scope(cfml_format, ds, indent_column, ds_start_column, anon_functs)
...
def is_multiline(cfml_format, ds, ds_start_column):
    ...
    multiline_min_length = cfml_format.get_setting(ds.scope_type + '.multiline.min_length')
...
def has_anon_funct_element(cfml_format, ds, ds_start_column, indent_column, anon_functs):
...
def render_ds_element(cfml_format, el, indent_column, ds_start_column, anon_functs):
    ...
    for part in el:
        if isinstance(part, DelimitedScope):
            ...
            el_str += render_delimited_scope(cfml_format, part, last_indent_column, last_start_column, anon_functs)
        else:
            el_str += cfml_format.view.substr(part)
...
def render_delimited_scope(cfml_format, ds, indent_column, ds_start_column, anon_functs):
    ...
    anon_funct_element = has_anon_funct_element(cfml_format, ds, ds_start_column, indent_column, anon_functs)
