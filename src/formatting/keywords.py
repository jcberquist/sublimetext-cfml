...
def format_keywords(cfml_format):
    if not cfml_format.get_setting("keywords"):
        return []
    ...
    for r in keywords:
        ...
        keyword_settings = cfml_format.get_setting("keywords", keyword)
        ...
        if not cfml_format.view.match_selector(previous_char_point, "comment"):
            ...
            if keyword in ["else", "elseif", "while", "catch"]:
                to_keyword_spacing = keyword_settings.get("block_to_keyword_spacing")
                if to_keyword_spacing in ["newline", "spaced", "compact"]:
                    previous_scope_name = cfml_format.view.scope_name(previous_char_point)
                    if previous_scope_name.strip().endswith(PREVIOUS_BLOCK_SCOPES[keyword] + " " + BLOCK_END_SELECTOR):
                        replacement_region = sublime.Region(previous_char_point + 1, r.begin())
                        replacement_str = get_spacing_string(cfml_format, previous_char_point, to_keyword_spacing)
                        substitutions.append((replacement_region, replacement_str))
            else:
                to_keyword_spacing = keyword_settings.get("to_keyword_spacing")
                if to_keyword_spacing in ["newline", "emptyline"]:
                    replacement_region = sublime.Region(previous_char_point + 1, r.begin())
                    replacement_str = get_spacing_string(cfml_format, r.begin(), to_keyword_spacing)
                    substitutions.append((replacement_region, replacement_str))
        ...
        if cfml_format.view.match_selector(next_char_pt, GROUP_START_SELECTOR):
            has_group = True
            spacing_to_group = keyword_settings.get("spacing_to_group")
            if spacing_to_group is not None and spacing_to_group in ["spaced", "compact"]:
            ...
            empty_group_spacing = keyword_settings.get("empty_group_spacing")
            padding_inside_group = keyword_settings.get("padding_inside_group")
            padding_strip_newlines = keyword_settings.get("padding_strip_newlines", False)
        ...
        next_char_is_block = cfml_format.view.match_selector(next_char_pt, block_selector)
        ...
        to_block_spacing = keyword_settings.get(setting_key)
        if to_block_spacing and to_block_spacing in ["newline", "spaced", "compact"]:
            replacement_str = get_spacing_string(cfml_format, prev_char_pt, to_block_spacing)
            substitutions.append((to_next_char, replacement_str))
...
def format_function_keywords(cfml_format):
    function_declaration = cfml_format.get_setting("function_declaration")
    ...
    spacing_to_group = function_declaration.get("spacing_to_group")
    if spacing_to_group and spacing_to_group in ["spaced", "compact"]:
        function_keywords = cfml_format.find_by_selector(FUNCTION_SELECTOR + "," + FUNCTION_NAME_SELECTOR)
    ...
    group_to_block_spacing = function_declaration.get("group_to_block_spacing")
    if group_to_block_spacing and group_to_block_spacing in ["newline", "spaced", "compact"]:
        param_end_regions = cfml_format.find_by_selector(FUNCTION_GROUP_END_SELECTOR)

        for r in param_end_regions:
            next_char_pt = utils.get_next_character(cfml_format.view, r.end())
            if cfml_format.view.match_selector(next_char_pt, FUNCTION_BLOCK_START_SELECTOR):
                group_to_block = sublime.Region(r.end(), next_char_pt)
                replacement_str = get_spacing_string(cfml_format, r.end(), group_to_block_spacing)
