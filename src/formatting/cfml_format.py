...
class CfmlFormatCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, current_method=True, settings=None):
        ...
        if self.view.window().project_file_name() and 'cfml_format' in self.view.window().project_data():
            self.settings['project_settings'] = self.view.window().project_data().get('cfml_format')
        ...
        total_time = 'CFML format completed in ' + '{:.0f}'.format((time.perf_counter() - start_time) * 1000) + 'ms'

        if (self.get_setting('log_command_times', default=False)):
            last_tick = start_time
            for cmd, tick in timing:
                print('CFML format: {} completed in {:.0f}ms'.format(cmd, (tick - last_tick) * 1000))
                last_tick = tick
    ...
    def get_setting(self, setting_key, sub_key=None, default=None):
        ...
        if sub_key is not None:
            ...
            full_key = '.'.join(setting_key_parts[:1] + [sub_key] + setting_key_parts[1:])
    ...
    def region_updates(self, edit, updates):
        ...
        slice_start, slice_end = self.region_to_format.begin(), self.region_to_format.end() + text_size_diff
