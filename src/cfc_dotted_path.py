...
def cfc_files(files):
    return [file_path for file_path in files if file_path.split(".")[-1].lower() == "cfc"]


def get_project_info(window):
    project_data = window.project_data()
    project_path = dirname(window.project_file_name()) if window.project_file_name() else None
...
class CfmlCfcDottedPathCommand(sublime_plugin.TextCommand):
    ...
    def is_visible(self):
        return self.view.file_name() is not None and len(cfc_files([self.view.file_name()])) == 1
