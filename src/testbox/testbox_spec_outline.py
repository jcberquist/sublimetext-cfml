...
testbox_function_names = ["describe", "it", "feature", "scenario", "story", "given", "when", "then"]
...
class TestboxSpecOutlineCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        # collect strings
        string_descriptions = {r.begin(): r for r in self.view.find_by_selector(string_scope)}
        ...
        for r in self.view.find_by_selector(func_call_scope):
            ...
            if next_pt in title_params:
                # get `=`
                next_pt = utils.get_next_character(self.view, title_params[next_pt].end())
            ...
            if next_pt in string_descriptions:
                ...
                depth = self.view.scope_name(r.begin()).count("meta.function.body.cfml") - 1
                indent = "    " * depth
                self.outline.append(indent + func_name + ": " + self.view.substr(string_region)[1:-1])
