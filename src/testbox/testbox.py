...
def get_dot_completions(cfml_view):
    ...
    # expectations
    if is_testbox_file(cfml_view) and len(cfml_view.dot_context) > 0 and cfml_view.dot_context[-1].name == "expect":
        return cfml_view.CompletionList(testbox["completions"]["expectation"], 1, False)

    # assertions
    if is_testbox_file(cfml_view) and len(cfml_view.dot_context) == 1 and cfml_view.dot_context[-1].name == "assert":
...
def get_script_completions(cfml_view):
    ...
    if is_testbox_file(cfml_view) and cfml_view.view.match_selector(cfml_view.position, "meta.class.body.cfml"):
