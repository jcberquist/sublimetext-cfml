...
def get_dot_completions(cfml_view):
    if not get_setting(cfml_view.view, "fw1_enabled") or len(cfml_view.dot_context) == 0:
        return None
    ...
    if get_file_type(cfml_view.view) == "controller":
        if len(cfml_view.dot_context) > 1 and cfml_view.dot_context[-2].name in ["renderdata", "renderer"]:
            return cfml_view.CompletionList(fw1["methods"]["renderdata"], 1, False)
...
def get_script_completions(cfml_view):
    ...
    if extends_fw1(cfml_view):
        if cfml_view.view.match_selector(cfml_view.position, "meta.class.body.cfml -meta.function.body -meta.struct-literal"):
...
def get_inline_documentation(cfml_view, doc_type):
    ...
    if view_extends_fw1 and cfml_view.view.match_selector(cfml_view.position, "meta.property"):
        context = cfml_view.get_dot_context(word_region.begin() - 1)

    if view_extends_fw1 and cfml_view.view.match_selector(cfml_view.position, "meta.class.body.cfml meta.struct-literal.cfml"):
