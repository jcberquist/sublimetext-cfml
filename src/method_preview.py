...
def add_preview_source(callback):
    preview_sources.append(callback)


def get_method_previews(cfml_view):
    previews = []

    for callback in preview_sources:
        preview = callback(cfml_view)
        if preview:
...
def _plugin_loaded():
    ...
    PREVIEW_TEMPLATE = sublime.load_resource(root_path + "/method_preview.html").replace("\r", "")
...
def generate_previews(docs, current_index):
    ...
    preview_html_variables["pagination"] = build_pagination(current_index, len(docs)) if len(docs) > 1 else ""
...
def display_previews(view, previews, pt=-1, current_index=0):
    ...
    if buffer_id in phantom_sets_by_buffer and pt != phantom_sets_by_buffer[buffer_id][1]:
        return
    ...
    view.add_regions("cfml_method_preview", merge_regions(preview_regions), "source", flags=sublime.DRAW_NO_FILL)

    phantom_set = sublime.PhantomSet(view, "cfml_method_preview")
    phantom_sets_by_buffer[buffer_id] = (phantom_set, pt)
    phantom = sublime.Phantom(view.line(pt), preview_html, sublime.LAYOUT_BLOCK, on_navigate)
...
class CfmlPreviewMethodCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, pt=None):
        ...
        if len(previews) > 0:
            display_previews(self.view, sorted(previews, key=lambda doc: doc.priority, reverse=True), position)
