...
# then uses the cfml_functions module to index the functions from those files. It assumes
# that all such files in the directory are component files, and does not check that the
# files actually define components. It returns a dict of normalized file paths mapped to
# a named tuple containing function metadata and completions.
...
# In my testing most time in the index function is spent walking the file system and reading
# files - so this is a test to see if offloading file string parsing to threads will allow
# file I/O wait time to be used for parsing and reduce the overall indexing time.

import os, re
from collections import namedtuple
from queue import Queue
from threading import Thread
from . import cfml_functions, cfml_properties

FunctionTuple = namedtuple("FunctionTuple", "name meta implicit")
PropertyTuple = namedtuple("PropertyTuple", "name meta")

cfc_regex = {"strings": {}, "booleans": {}}
for key in ["extends","initmethod","entityname"]:
  cfc_regex["strings"][key] = re.compile("component[^>{]*" + key + "\\s*=\\s*[\"']?([a-z0-9:.]+)", re.I)
for key in ["accessors","persistent"]:
  cfc_regex["booleans"][key] = re.compile("component[^>{]*" + key + "\\s*=\\s*[\"']?(true|yes)", re.I)
...
def parse_cfc_file_string(file_string):
  cfc_index = {}
  cfc_index.update(cfc_string_attributes(file_string))
  cfc_index.update(cfc_boolean_attributes(file_string))
...
def cfc_string_attributes(file_string):
  attrs = {}
  for key in cfc_regex["strings"]:
    attr_search = re.search(cfc_regex["strings"][key], file_string)
    attrs[key] = attr_search.group(1) if attr_search else None
  return attrs

def cfc_boolean_attributes(file_string):
  attrs = {}
  for key in cfc_regex["booleans"]:
    attr_search = re.search(cfc_regex["booleans"][key], file_string)
    attrs[key] = True if attr_search else False
  return attrs
