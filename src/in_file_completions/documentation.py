...
def get_method_preview(cfml_view):
    ...
    if extended_meta:
        ...
        view_file_path = utils.normalize_path(cfml_view.view.file_name()) if cfml_view.view.file_name() else ""
        if len(function_file_path) > 0 and function_file_path != view_file_path:
            doc, callback = component_index.get_method_preview(cfml_view.view, cfml_view.project_name, function_file_path, function_name)
        else:
            function_preview = builders.build_method_preview(cfml_view.view, function_name)
            doc = builders.build_method_preview_doc(extended_meta, function_name, function_preview)
            callback = partial(on_navigate, cfml_view.view, function_file_path, extended_meta["function_file_map"])
...
def get_completion_docs(cfml_view):
    if cfml_view.function_call_params and not cfml_view.function_call_params.support:
        ...
        if header:
            model_doc = builders.build_function_call_params_doc(cfml_view.view_metadata, cfml_view.function_call_params, None, header)
...
def get_function_call_params_header(cfml_view):
    if not cfml_view.function_call_params.method:
        if cfml_view.function_call_params.function_name in cfml_view.view_metadata["functions"]:
            return cfml_view.view_metadata["functions"][cfml_view.function_call_params.function_name]["name"]
    else:
...
def get_cfc_info(cfml_view, pt):
    ...
    if cfml_view.view.match_selector(pt, "variable.language.super.cfml"):
        extends_file_path = component_index.resolve_path(cfml_view.project_name, file_path, cfml_view.view_metadata["extends"])
        extended_meta = component_index.get_extended_metadata_by_file_path(cfml_view.project_name, extends_file_path)
        if extended_meta:
...
def get_function_documentation(view, project_name, extended_meta, function_name, header):
    ...
    if len(function_file_path) > 0 and function_file_path != view_file_path:
        model_doc, callback = component_index.get_method_documentation(view, project_name, function_file_path, function_name, None, header)
    else:
        ...
        model_doc = builders.build_method_documentation(extended_meta, function_name, None, header, function_preview)
