...
def get_script_completions(cfml_view):
    ...
    if cfml_view.view.match_selector(cfml_view.position, "meta.function-call.support.entity.cfml meta.function-call.parameters.support.cfml string.quoted"):
...
def get_dot_completions(cfml_view):
    ...
    if (
        cfml_view.dot_context[0].name in entity_methods and
        cfml_view.view.match_selector(cfml_view.prefix_start - 2, entity_selector)
    ):
        entity_name = entity_utils.get_entity_name(cfml_view.view.substr(cfml_view.dot_context[0].args_region), cfml_view.dot_context[0].name)

    elif entity_utils.is_possible_entity(cfml_view.dot_context):
        entity_tuple = entity_utils.find_entity_by_var_assignment(cfml_view, cfml_view.prefix_start, cfml_view.dot_context[0].name)
        if entity_tuple[0] is not None:
            entity_name = entity_tuple[0]

    if entity_name:
        completions = get_completions_by_entity_name(cfml_view.project_name, entity_name)
        if completions:
