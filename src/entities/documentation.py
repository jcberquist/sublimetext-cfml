...
def get_inline_documentation(cfml_view, doc_type):
    ...
    if entity_name:
        file_path_info = component_index.get_file_path_by_entity_name(cfml_view.project_name, entity_name)
        if file_path_info is None:
            return None

        if function_name:
            metadata = component_index.get_extended_metadata_by_file_path(cfml_view.project_name, file_path_info["file_path"])
