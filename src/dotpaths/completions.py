...
def get_tag_attributes(cfml_view):
    ...
    if cfml_view.view.match_selector(cfml_view.position - 1, "meta.class.inheritance.cfml -entity.other.inherited-class.cfml"):
        cfc_path = ""
        folder_cfc_path = cfc_utils.get_folder_cfc_path(cfml_view, cfc_path)

        completions = []
        completions.extend(get_completions(cfml_view.project_name, cfc_path, "path_completions"))
        completions.extend(get_completions(cfml_view.project_name, folder_cfc_path, "path_completions"))

        if len(completions) > 0:
            return cfml_view.CompletionList(completions, 2, True)

    if cfml_view.view.match_selector(cfml_view.position - 1, "entity.other.inherited-class.cfml"):
        r = utils.get_scope_region_containing_point(cfml_view.view, cfml_view.position - 1, "entity.other.inherited-class.cfml")
...
def get_script_completions(cfml_view):
    ...
    if cfml_view.view.match_selector(cfml_view.position, "meta.function-call.support.createcomponent.cfml string.quoted"):
        r = utils.get_scope_region_containing_point(cfml_view.view, cfml_view.position, "string.quoted")
    ...
    if cfml_view.view.match_selector(cfml_view.position - 1, "meta.instance.constructor.cfml"):
        r = utils.get_scope_region_containing_point(cfml_view.view, cfml_view.position - 1, "meta.instance.constructor.cfml")
        r = sublime.Region(r.begin(), cfml_view.position - len(cfml_view.prefix))
        cfc_path = ".".join(cfml_view.view.substr(r)[4:].split(".")[:-1])
        folder_cfc_path = cfc_utils.get_folder_cfc_path(cfml_view, cfc_path)

        completions = []
        completions.extend(get_completions(cfml_view.project_name, cfc_path, "constructor_completions"))
        completions.extend(get_completions(cfml_view.project_name, folder_cfc_path, "constructor_completions"))
...
def get_dot_completions(cfml_view):
    ...
    if (cfml_view.dot_context[0].name == "createobject" and cfml_view.view.match_selector(cfml_view.prefix_start - 2, component_selector)):
        component_name = cfc_utils.get_component_name(cfml_view.view.substr(cfml_view.dot_context[0].args_region))
    elif cfml_view.view.match_selector(cfml_view.prefix_start - 2, constructor_selector):
    ...
    if comp:
        ...
        for completion in comp["functions"]:
            if not completion.private:
                filtered_completions.append((completion.key + "\t" + completion.hint, completion.content))
